...
void ofApp::setup(){
    ...
    string fixed_point = ad_util::data_path + "img/fixed_point/lg/losglaciares12.jpg";
    string gns2 =  "img/photo/gns2.jpg";
...
void ofApp::setVertices(){
    ...
    for( int i=0; i<vs.size(); i++ ){
        ...
        speeds[i].z += gn1.getNoisef(i,2) * amp1 * b * 0.01;
        nd1[i] += speeds[i];
...
//    mesh.clearIndices();
	...
	for( int i=0; i<vs.size(); i++ ){
        ...
        if( gn1.getNoiseuf(index*3,0)>0.4 ){
            
            mesh.removeIndex( index*3 );
            mesh.removeIndex( index*3  );
            mesh.removeIndex( index*3 );
