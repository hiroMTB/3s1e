...
//------------------------------------
template<typename PixelType>
void ofImage_<PixelType>::grabScreen(int _x, int _y, int _w, int _h, bool _alpha){

    if( _alpha )
        allocate(_w, _h, OF_IMAGE_COLOR_ALPHA);
    else
    ...
    if(ofGetOrientation() == OF_ORIENTATION_DEFAULT || ofDoesHWOrientation()) {
        ...
        for(int y = 0; y < _h; y++){  
            for(int x = 0; x < _w; x++){
                ...
                if(_alpha) pixels.getPixels()[i+3] = bufferRGBA[j+3];
            }
        }
    }
